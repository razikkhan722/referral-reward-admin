  ...
  return (
    <div className={`mb-3 ${className}`}>
      ...
      <Dropdown onSelect={onChange}>
        <Dropdown.Toggle
          variant="light"
          className={`w-100 form-select text-blue-color rounded-3 border-0 py-2 d-flex justify-content-between align-items-center ${error ? "border border-danger" : ""}`}
        >
          <span className="font-14 montserrat-medium">{value || placeholder}</span>
          <IoIosArrowDown className="text-blue-color" />
        </Dropdown.Toggle>
        <Dropdown.Menu className="w-100 shadow-sm rounded">
          {options.map((option) => (
